...
from src.apps.clothes.models import Clothe, LaundrySchedule
from src.apps.clothes.forms import AddClotheForm
import json
...
def upload_progress(request):
    ...
    if progress_id:
        ...
        return HttpResponseServerError(_('Server Error: You must provide X-Progress-ID header or query param.'))
...
def add_cloth(request):
	if request.user.is_authenticated:
		...
		if request.method == 'POST':
			...
			count_success_obj = 0
			count_error_obj = 0
			clothes_images = request.FILES.getlist('image')
			form = AddClotheForm(request.POST, request.FILES)
			...
			return HttpResponseRedirect(reverse('clothes:add_cloth'))
		else:
			user_clothes = Clothe.objects.filter(user=request.user).order_by('-timestamp')
...
def room_dirty_bucket(request):
    if request.user.is_authenticated:
        context = {}
        dirty_clothes = Clothe.objects.filter(is_dirty=True, is_laundry=False)
        context.update({
